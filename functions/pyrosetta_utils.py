...
from .biopython_utils import hotspot_residues, hotspot_residues_multi
...
# Rosetta interface scores
def score_interface_multi(pdb_file, binder_chain="B", target_chains="A,C"):
    # load pose
    pose = pr.pose_from_pdb(pdb_file)

    interacting_res_all, per_chain_contacts = hotspot_residues_multi(
        pdb_file,
    ...
    for target_chain in [c.strip() for c in target_chains.split(",") if c.strip()]:
        # analyze interface statistics
        iam = InterfaceAnalyzerMover()
        ...
        # Initialize list to store PDB residue IDs at the interface
        res_ids = per_chain_contacts.get(target_chain, set())
        interface_residues_set = {res_id: interacting_res_all[res_id]
                                  for res_id in res_ids
                                  if res_id in interacting_res_all}
        interface_residues_pdb_ids = []
        ...
        interface_scores_all[target_chain] = interface_scores
        interface_AA_all[target_chain] = interface_AA
        interface_residues_all[target_chain] = interface_residues_pdb_ids_str


    return interface_scores_all, interface_AA_all, interface_residues_all
