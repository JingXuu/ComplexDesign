...
def split_multichain_by_lengths(seq, lengths):
    ...
    if "/" in seq:
        ...
        for part, expected_len in zip(parts, lengths):
            if len(part) != expected_len:
                raise ValueError("chain length mismatch")
        return parts
    ...
    parts = []
    start = 0
    for chain_len in lengths:
        parts.append(seq[start:start + chain_len])
        start += chain_len
    return parts

def save_multichain_fasta(design_name, chain_parts, design_paths):
    fasta_path = os.path.join(design_paths["MPNN/Sequences"], design_name + ".fasta")
    chain_ids = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    with open(fasta_path, "w") as fasta:
        if len(chain_parts) == 1:
            fasta.write(f">{design_name}\n{chain_parts[0]}\n")
        else:
            for chain_id, seq in zip(chain_ids, chain_parts):
                fasta.write(f">{design_name}|chain{chain_id}\n{seq}\n")
...
# Check if we have required number of accepted targets, rank them, and analyse sequence and structure properties
def check_accepted_designs_multi(design_paths, mpnn_csv_map, final_labels, final_csv_map, advanced_settings, target_settings, design_labels):
    accepted_binders = [f for f in os.listdir(design_paths["Accepted"]) if f.endswith('.pdb') and not f.startswith('.')]
    ...
    if len(accepted_binders) >= target_settings["number_of_final_designs"]:
        ...
        first_chain = True
        for chain_id, mpnn_csv in mpnn_csv_map.items():
            if first_chain:
                # clear the Ranked folder in case we added new designs in the meantime so we rerank them all
                for f in os.listdir(design_paths["Accepted/Ranked"]):
                    os.remove(os.path.join(design_paths["Accepted/Ranked"], f))
        ...
        # save the final_df to final_csv
        # final_df.to_csv(final_csv, index=False)
            final_csv = final_csv_map[chain_id]
            final_df.to_csv(final_csv, index=False)
...
def perform_input_check(args):
    ...
    if not args.advanced:
        args.advanced = os.path.join(binder_script_path, 'settings_advanced', 'default_4stage_complexdesign_multitarget.json')

    return args.settings, args.filters, args.advanced

# check specific advanced settings
def perform_advanced_settings_check(advanced_settings, complexdesign_folder):
    # set paths to model weights and executables
    if complexdesign_folder == "colab":
        advanced_settings["af_params_dir"] = '/content/complexdesign/params/'
        advanced_settings["dssp_path"] = '/content/complexdesign/functions/dssp'
        advanced_settings["dalphaball_path"] = '/content/complexdesign/functions/DAlphaBall.gcc'
    else:
        # Set paths individually if they are not already set
        if not advanced_settings["af_params_dir"]:
            advanced_settings["af_params_dir"] = complexdesign_folder
        if not advanced_settings["dssp_path"]:
            advanced_settings["dssp_path"] = os.path.join(complexdesign_folder, 'functions', 'dssp')
        if not advanced_settings["dalphaball_path"]:
            advanced_settings["dalphaball_path"] = os.path.join(complexdesign_folder, 'functions', 'DAlphaBall.gcc')
