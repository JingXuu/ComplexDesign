...
from .biopython_utils import hotspot_residues, calculate_clash_score, calc_ss_percentage, calculate_percentages, hotspot_residues_multi, calc_ss_percentage_multi
from .pyrosetta_utils import pr_relax, align_pdbs
from .generic_utils import (
    update_failures,
    split_multichain_by_lengths,
)
from .multi_utils import count_chain_residues, restore_AC_chains

def collect_mpnn_sequences_multitarget(
    trajectory_pdb,
    binder_chain,
    ...
    num_target = int(advanced_settings["num_seqs"])

    if advanced_settings.get("force_reject_AA", False):
        omit = advanced_settings.get("omit_AAs", "")
        restricted_AAs = {
            aa.strip().upper() for aa in omit.split(",") if aa.strip()
        }
    else:
        restricted_AAs = set()
    ...
    for _ in range(max_rounds):
        if len(seq_map) >= num_target:
            break

        mpnn_trajectories = mpnn_gen_sequence_multi(
            trajectory_pdb,
            binder_chain,
            trajectory_interface_residues,
            advanced_settings,
        ...
        seqs = mpnn_trajectories.get("seq", [])
        scores = mpnn_trajectories.get("score", [])
        seqids = mpnn_trajectories.get("seqid", [])
        max_idx = min(len(seqs), num_target)
        ...
        for n in range(max_idx):
            ...
            seq = full_seq[-target_length:].upper()
            if restricted_AAs and any(aa in seq for aa in restricted_AAs):
                continue

            score = float(scores[n])
            seqid = float(seqids[n])
            if seq not in seq_map or score < seq_map[seq]["score"]:
                seq_map[seq] = {"seq": seq, "score": score, "seqid": seqid}

    return sorted(seq_map.values(), key=lambda x: x["score"])[:num_target]
...
def binder_hallucination(design_name, starting_pdb, chain, target_hotspot_residues, length, seed, helicity_value, design_models, advanced_settings, design_paths, failure_csv):
    ...
    af_model = mk_af_model(protocol="binder", debug=False, data_dir=advanced_settings["af_params_dir"], 
...
# hallucinate a binder
def binder_hallucination_multi_target(design_name, starting_pdb, chain, target_hotspot_residues, length, seed, helicity_value, design_models, advanced_settings, design_paths, failure_csv):
    model_pdb_path = os.path.join(design_paths["Trajectory"], design_name+".pdb")
    ...
    chain_res_count = count_chain_residues(starting_pdb)
    ...
    if advanced_settings["design_algorithm"] == '2stage':
        ...
        if initial_plddt > 0.65:
            ...
            if advanced_settings["optimise_beta"]:
                af_model.save_pdb(model_pdb_path)
                restore_AC_chains(model_pdb_path, model_pdb_path, chain_res_count)

                _, beta, *_ = calc_ss_percentage_multi(model_pdb_path, advanced_settings, 'B')
                os.remove(model_pdb_path)
    ...
    if ca_clashes > 0:
        ...
        if final_plddt < 0.65:
            ...
            binder_contacts, _ = hotspot_residues_multi(
                model_pdb_path,
                target_chains="A,C",
                atom_distance_cutoff=3.5,
...
# run prediction for binder with masked template target
def predict_binder_complex_multi(prediction_model, binder_sequence, mpnn_design_name, target_pdb, chain, length, trajectory_pdb, prediction_models, advanced_settings, filters, design_paths, failure_csv, seed=None):
    prediction_stats = {}
    ...
    chain_res_count = count_chain_residues(target_pdb)
    ...
    for model_num in prediction_models:
        ...
        if True:
            ...
            prediction_model.predict(seq=binder_sequence, models=[model_num], num_recycles=advanced_settings["num_recycles_validation"], verbose=False)
            prediction_model.save_pdb(complex_pdb)
            restore_AC_chains(complex_pdb, complex_pdb, chain_res_count)
            prediction_metrics = copy_dict(prediction_model.aux["log"]) # contains plddt, ptm, i_ptm, pae, i_pae
...
# run MPNN to generate sequences for binders
def mpnn_gen_sequence_multi(trajectory_pdb, binder_chain, trajectory_interface_residues, advanced_settings, target_chains="A,C"):
    # clear GPU memory
    clear_mem()
    ...
    # check whether keep the interface generated by the trajectory or whether to redesign with MPNN
    design_chains = target_chains + ',' + binder_chain

    if advanced_settings["mpnn_fix_interface"]:
        fixed_positions = target_chains + ','  + trajectory_interface_residues
        fixed_positions = fixed_positions.rstrip(",")
        print("Fixing interface residues: "+trajectory_interface_residues)
    else:
...
def mpnn_gen_sequence_complex(trajectory_pdb, lengths, advanced_settings):
    # clear GPU memory
    clear_mem()
    ...
    if n_chains > len(chain_ids):
        raise ValueError(f"mpnn_gen_sequence_complex: too many chains ({n_chains})")
    design_chains = ",".join(list(chain_ids[:n_chains]))  # e.g. "A,B" or "A,B,C"
...
def collect_mpnn_sequences_unconditional(trajectory_pdb, lengths, advanced_settings):
    num_target = int(advanced_settings["num_seqs"])
    ...
    mpnn_trajectories = mpnn_gen_sequence_complex(
    ...
    for idx, full_seq in enumerate(seqs):
        if full_seq is None:
            continue

        seq = full_seq.replace(" ", "").replace("\n", "").strip().upper()
        if restricted_AAs and any(aa in seq for aa in restricted_AAs):
            continue

        try:
            parts = split_multichain_by_lengths(seq, lengths)
        except ValueError:
            continue

        key = "/".join(parts)
        score = float(scores[idx])
        seqid = float(seqids[idx])
        if key not in seq_map or score < seq_map[key]["score"]:
            seq_map[key] = {
                "seq": key,
                "parts": parts,
                "score": score,
...
def complex_hallucination_unconditional(
    design_name,
    lengths,
    seed,
    design_models,
    ...
    if not use_multimer:
        print("[WARN] complex hallucination requires use_multimer=True, overriding.")
        use_multimer = True
    ...
    af_model.opt["weights"].update(
        {
            ...
            "con": advanced_settings["weights_con_intra"],
            "i_con": advanced_settings["weights_con_inter"],
            "helix": advanced_settings["weights_helicity"],
        }
    )
    ...
    greedy_tries = math.ceil(
        total_len * (advanced_settings["greedy_percentage"] / 100.0)
    )
    ...
    if advanced_settings["design_algorithm"] == "2stage":
        ...
        initial_plddt = get_best_plddt(af_model, total_len)
        ...
        if initial_plddt > 0.65:
            ...
            if logits_iter > 0:
                ...
                af_model._tmp["seq_logits"] = af_model.aux["seq"]["logits"]
                logit_plddt = get_best_plddt(af_model, total_len)
                print("Optimised logit trajectory pLDDT: " + str(logit_plddt))
            else:
                logit_plddt = initial_plddt
            ...
            if advanced_settings["temporary_iterations"] > 0:
                ...
                softmax_plddt = get_best_plddt(af_model, total_len)
            else:
                softmax_plddt = logit_plddt
            ...
            if softmax_plddt > 0.65:
                ...
                if advanced_settings["hard_iterations"] > 0:
                    ...
                    onehot_plddt = get_best_plddt(af_model, total_len)
    ...
    final_plddt = get_best_plddt(af_model, total_len)
    af_model.save_pdb(model_pdb_path)
