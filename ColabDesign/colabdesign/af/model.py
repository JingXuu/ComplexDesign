...
class mk_af_model(design_model, _af_inputs, _af_loss, _af_prep, _af_design, _af_utils):
    def __init__(
        ...
        **kwargs,
        ...
        if self._args["use_multimer"]:
            ...
            self._cfg = config.model_config(
                "model_1_ptm" if self._args["use_templates"] else "model_3_ptm"
            )
        ...
        for model_name in model_names:
            params = data.get_model_haiku_params(
                model_name=model_name, data_dir=data_dir, fuse=True
            )
    ...
    def _get_model(self, cfg, callback=None):
        ...
        runner = model.RunModel(
            cfg, recycle_mode=a["recycle_mode"], use_multimer=a["use_multimer"]
        )
        ...
        def _model(params, model_params, inputs, key):
            ...
            aux.update(
                {
                    "atom_positions": outputs["structure_module"][
            ...
            if a["use_mlm"]:
                ...
                aux["losses"].update(
                    get_mlm_loss(outputs, mask=mask, truth=seq["pssm"])
                )
        ...
        return {
            "grad_fn": jax.jit(jax.value_and_grad(_model, has_aux=True, argnums=0)),
            "fn": jax.jit(_model),
            "runner": runner,
        }
