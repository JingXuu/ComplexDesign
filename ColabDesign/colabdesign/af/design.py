...
class _af_design:
    ...
    def run(
        ...
        if self.protocol in ["fixbb", "partial"] or (
            self.protocol == "binder" and self._args["redesign"]
        ):
        ...
        self.aux["log"].update(
            {"recycles": int(self.aux["num_recycles"]), "models": model_nums}
        )
            ...
            if "prev" not in self._inputs or a["clear_prev"]:
                ...
                if a["use_initial_atom_pos"]:
                    if "batch" in self._inputs:
                        self._inputs["initial_atom_pos"] = self._inputs["batch"][
                            "all_atom_positions"
                        ]
                    else:
    ...
    def _save_results(
        ...
        if save_best:
            ...
            if (
                self._args["best_metric"]
                in ["plddt", "ptm", "i_ptm", "seqid", "composite"]
                or metric_higher_better
            ):
                metric = -metric
            if (
                "metric" not in self._tmp["best"]
                or metric < self._tmp["best"]["metric"]
            ):
        ...
        **kwargs,
        ...
        def load_settings():
            if "save" in self._tmp:
                [self.opt, self._args, self._params, self._inputs] = self._tmp.pop(
                    "save"
                )
        ...
        def save_settings():
            load_settings()
            self._tmp["save"] = [
                copy_dict(x) for x in [self.opt, self._args, self._params, self._inputs]
            ]
            ...
            for k, (s, e) in m.items():
                if k == "temp":
                    ...
                    if k == "step":
                        ...
                        num_recycles = round(v)
                    else:
                        self.set_opt({k: v})

            # decay learning rate based on temperature
            lr_scale = step * (
                (1 - self.opt["soft"]) + (self.opt["soft"] * self.opt["temp"])
            )
    ...
    def design_3stage(
        ...
        **kwargs,
        ...
        if soft_iters > 0:
            ...
            self.design_logits(
                soft_iters, e_soft=1, ramp_recycles=ramp_recycles, **kwargs
            )
    ...
    def design_semigreedy(
        ...
        **kwargs,
        ...
        plddt = (
            plddt[self._target_len :]
            if self.protocol == "binder"
            else plddt[: self._len]
        )
        ...
        for i in range(iters):
            ...
            for t in range(int(num_tries)):
                mut_seq = self._mutate(
                    seq=seq, plddt=plddt, logits=seq_logits + self._inputs["bias"]
                )
                aux = self.predict(
                    seq=mut_seq,
                    return_aux=True,
