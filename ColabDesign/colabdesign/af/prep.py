...
class _af_prep:
    ...
    def _prep_fixbb(
        ...
        **kwargs,
        ...
        if copies > 1:
            ...
            if repeat or homooligomer:
                ...
                if "fix_pos" in self.opt:
                    self.opt["fix_pos"] = self.opt["fix_pos"][
                        self.opt["fix_pos"] < self._len
                    ]
        ...
        self._inputs["batch"] = make_fixed_size(
            self._pdb["batch"], num_res=sum(self._lengths)
        )
        self._inputs.update(get_multi_id(self._lengths, homooligomer=homooligomer))

        # configure options/weights
        self.opt["weights"].update(
            {"dgram_cce": 1.0, "rmsd": 0.0, "fape": 0.0, "con": 0.0}
        )
    ...
    def _prep_hallucination(self, length=100, copies=1, repeat=False, lengths=None, **kwargs):
        """
        ...
        if lengths is not None:
            ...
            self.opt["weights"].update({"con": 1.0, "i_con": 1.0})
            # NOTE: use i_pae define out 

            offset = 50
            res_idx_list = []
            for i, L in enumerate(self._lengths):
                if i == 0:
                    res_idx_chain = np.arange(L)
                else:
                    prev_last = res_idx_list[-1][-1]
                    res_idx_chain = prev_last + np.arange(L) + offset
                res_idx_list.append(res_idx_chain)
            res_idx = np.concatenate(res_idx_list)

            self._inputs = self._prep_features(num_res=sum(self._lengths), num_seq=num_seq)
            self._inputs["residue_index"] = res_idx
            self._inputs.update(get_multi_id(self._lengths, homooligomer=False))

            self._inputs["mask_ac_interchain"] = False
        ...
        self._args.update(
            {"repeat": repeat, "block_diag": block_diag, "copies": copies}
        )
    ...
    def _prep_binder(
        ...
        **kwargs,
        ...
        if redesign:
            im += [ignore_missing] * len(binder_chain.split(","))

        self._pdb = prep_pdb(pdb_filename, chain=chains, ignore_missing=im)
        res_idx = self._pdb["residue_index"]

        if redesign:
            self._target_len = sum(
                [
                    (self._pdb["idx"]["chain"] == c).sum()
                    for c in target_chain.split(",")
                ]
            )
            self._binder_len = sum(
                [
                    (self._pdb["idx"]["chain"] == c).sum()
        ...
        if redesign:
            ...
            self._pdb["batch"] = make_fixed_size(
                self._pdb["batch"], num_res=sum(self._lengths)
            )
            self.opt["weights"].update(
                {"plddt": 0.1, "con": 0.0, "i_con": 1.0, "i_pae": 0.0}
            )
        # configure input features
        ...
        if redesign:
            af_lengths = self._lengths
        else:
            af_lengths = list(self._pdb["lengths"]) + [self._binder_len] # [len_A, len_C, len_B]

        self._inputs.update(get_multi_id(af_lengths)) # asym_id:[0...0,1...1,2...2]
        self._inputs["mask_ac_interchain"] = True
    ...
    def _prep_partial(
        ...
        **kwargs,
        ...
        self._inputs = self._prep_features(num_res=sum(self._lengths), num_seq=num_seq)
        self._inputs["residue_index"] = res_idx
        self._inputs["batch"] = jax.tree_util.tree_map(
            lambda x: x[self._pdb["pos"]], self._pdb["batch"]
        )
        self._inputs.update(get_multi_id(self._lengths, homooligomer=homooligomer))

        # configure options/weights
        self.opt["weights"].update(
            {"dgram_cce": 1.0, "rmsd": 0.0, "fape": 0.0, "con": 1.0}
        )
        ...
        if use_sidechains:
            self._sc = {
                "batch": prep_inputs.make_atom14_positions(self._inputs["batch"]),
...
def repeat_pos(pos, copies, length):
    return (
        np.repeat(pos, copies).reshape(-1, copies) + np.arange(copies) * length
    ).T.flatten()


def prep_pdb(
    pdb_filename, chain=None, offsets=None, lengths=None, ignore_missing=False
):
    ...
    def add_cb(batch):
        ...
        batch["all_atom_positions"][..., cb, :] = np.where(
            m[:, cb, None], p[:, cb, :], cb_atoms
        )
    ...
    for n, chain in enumerate(chains):
        ...
        if im:
            ...
            offset = (
                0
                if offsets is None
                else (offsets[n] if isinstance(offsets, list) else offsets)
            )
            r = offset + (protein_obj.residue_index - protein_obj.residue_index.min())
            length = (
                (r.max() + 1)
                if lengths is None
                else (lengths[n] if isinstance(lengths, list) else lengths)
            )
        ...
        o.append({"batch": batch, "residue_index": residue_index, "cb_feat": cb_feat})
    ...
    o["idx"] = {
        "residue": np.concatenate(residue_idx),
...
def get_sc_pos(aa_ident, atoms_to_exclude=None):
    ...
    for r in resname_to_idx:
        if isinstance(atoms_to_exclude, dict):
            a2e[r] = atoms_to_exclude.get(
                r, atoms_to_exclude.get("ALL", ["N", "C", "O"])
            )
        else:
