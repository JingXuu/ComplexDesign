...
class _af_loss:
    ...
    def _loss_fixbb(self, inputs, outputs, aux):
        ...
        if self._args["realign"]:
            aux["atom_positions"] = (
                aln["align"](aux["atom_positions"]) * aux["atom_mask"][..., None]
            )
        ...
        aux["losses"].update(
            {
                "fape": get_fape_loss(
                    inputs, outputs, copies=copies, clamp=opt["fape_cutoff"]
                ),
                "dgram_cce": get_dgram_loss(
                    inputs, outputs, copies=copies, aatype=inputs["aatype"]
                ),
                "rmsd": aln["rmsd"],
            }
        )
    ...
    def _loss_binder(self, inputs, outputs, aux):
        ...
        aux["losses"].update(
            {
                "plddt": get_plddt_loss(
                    outputs, mask_1d=binder_id
                ),  # plddt over binder
                "exp_res": get_exp_res_loss(outputs, mask_1d=binder_id),
                "pae": get_pae_loss(
                    outputs, mask_1d=binder_id
                ),  # pae over binder + interface
                "con": get_con_loss(
        ...
        if self._args["redesign"]:
            ...
            # compute fape
            fape = get_fape_loss(
                inputs, outputs, clamp=opt["fape_cutoff"], return_mtx=True
            )
        ...
        if self._args["realign"]:
            aux["atom_positions"] = (
                align_fn(aux["atom_positions"]) * aux["atom_mask"][..., None]
            )
    ...
    def _loss_partial(self, inputs, outputs, aux):
        ...
        dgram = {
            "logits": sub(sub(outputs["distogram"]["logits"]), 1),
        ...
        if self._args["use_sidechains"] and copies == 1:
            ...
            true_pos = all_atom.atom37_to_atom14(
                inputs["batch"]["all_atom_positions"], self._sc["batch"]
            )
            ...
            if not self._args["use_multimer"]:
                sc_struct = {
                    **folding.compute_renamed_ground_truth(self._sc["batch"], pred_pos),
                    "sidechains": {
                        k: sub(struct["sidechains"][k], 1)
                ...
                aux["losses"]["sc_fape"] = folding.sidechain_loss(
                    batch, sc_struct, self._cfg.model.heads.structure_module
                )["loss"]
    ...
    def _loss_unsupervised(self, inputs, outputs, aux):
        ...
        if "pos" in opt:
            ...
            if C > 1:
                pos = (
                    jnp.repeat(pos, C).reshape(-1, C) + jnp.arange(C) * L
                ).T.flatten()
        ...
        masks = {"mask_1d": mask_1d, "mask_2d": jnp.where(seq_mask_2d, mask_2d, 0)}
        ...
        has_diffchain = len(getattr(self, "_lengths", [])) > 1
        has_interface = (self._args["copies"] > 1 and not self._args["repeat"]) or has_diffchain


        # define losses at interface
        if has_interface:
            inter_mask_2d = jnp.where(seq_mask_2d, mask_2d == False, 0)
            masks = {
                "mask_1d": (
                    mask_1d if self._args["homooligomer"] else inputs["seq_mask"]
                ),
                "mask_2d": inter_mask_2d
            }
            losses.update(
                {
...
def get_con_loss(inputs, outputs, con_opt, mask_1d=None, mask_1b=None, mask_2d=None):
    ...
    # mask results
    if mask_1d is None:
        mask_1d = jnp.ones(m.shape[0])
    if mask_1b is None:
        mask_1b = jnp.ones(m.shape[0])
...
def get_helix_loss(inputs, outputs):
    ...
    seq_mask_2d = inputs["seq_mask"][:, None] * inputs["seq_mask"][None, :]
    same_chain_2d = inputs["asym_id"][:, None] == inputs["asym_id"][None, :]
    mask_2d = jnp.where(seq_mask_2d, same_chain_2d, 0)
...
def _get_helix_loss(dgram, dgram_bins, offset=None, mask_2d=None, **kwargs):
    ...
    if offset is None:
        if mask_2d is None:
            ...
            return jnp.diagonal(x * mask_2d, 3).sum() + (
...
def get_dgram_loss(inputs, outputs, copies=1, aatype=None, return_mtx=False):
    ...
    return _get_pw_loss(
        true, pred, loss_fn, weights=weights, copies=copies, return_mtx=return_mtx
    )
...
def get_fape_loss(inputs, outputs, copies=1, clamp=10.0, return_mtx=False):
    ...
    true_mask = jnp.where(
        inputs["seq_mask"][:, None], inputs["batch"]["all_atom_mask"], 0
    )
...
def get_rmsd_loss(inputs, outputs, L=None, include_L=True, copies=1):
    ...
    return _get_rmsd_loss(
        true, pred, weights=weights, L=L, include_L=include_L, copies=copies
    )
...
def _get_rmsd_loss(true, pred, weights=None, L=None, include_L=True, copies=1):
    ...
    if iL == 0:
        ...
        msd = (
            msd_fn(true, pred, weights)
            if include_L
            else (msd_fn(iT, iP, iW) / iW.sum((-1, -2)))
        )
